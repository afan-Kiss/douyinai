...
def _normalize_uid(raw: str) -> str:
    ...
    if not uid.startswith("AQ"):
        return ""
    # Official security_uid is 88 chars; binary scans may include suffix noise.
    if len(uid) > 88:
        uid = uid[:88]
...
def _user_card_hint(session, uid: str) -> dict[str, Any]:
    ...
    name = str(inner.get("user_name") or inner.get("nick_name") or inner.get("user_from_desc") or "")
    preview = ""
    if deal > 0:
        preview = f"成交{deal}单"
    elif repeat > 0:
        preview = f"复访{repeat}次"
    return {"name": name, "preview": preview, "card": inner}
...
def list_conversations_fallback(session, *, limit: int = 30) -> dict[str, Any]:
    ...
    for uid in uids[:limit]:
        ...
        preview = str(hint.get("preview") or "已知买家（xundan 11001 fallback）")
        items.append(
            {
