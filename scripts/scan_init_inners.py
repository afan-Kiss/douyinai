#!/usr/bin/env python3
"""Scan get_message_by_init response for 169B signature material."""
from __future__ import annotations

import json
import re
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "src"))

OUT = ROOT / "analysis" / "init_inner_scan.json"


def main() -> int:
    from pigeon_protocol.foundation.ws_inner_bootstrap import scan_binary_for_inners
    from pigeon_protocol.feige_init import GET_MESSAGE_BY_INIT, _load_init_bytes, _patch_init_body
    from pigeon_protocol.foundation.chrome_hints import pigeon_im_headers
    from pigeon_protocol.http_client import DEFAULT_CURL_IMPERSONATE
    from pigeon_protocol.pigeon_im import build_pigeon_im_url
    from pigeon_protocol.session import load_session
    from curl_cffi import requests as curl_requests

    session = load_session()
    body = _patch_init_body(_load_init_bytes(), session)
    url = build_pigeon_im_url(session, GET_MESSAGE_BY_INIT, sign=True)
    resp = curl_requests.post(
        url,
        data=body,
        headers=pigeon_im_headers(session),
        impersonate=DEFAULT_CURL_IMPERSONATE,
        timeout=20,
    )
    raw = resp.content
    hits = scan_binary_for_inners(raw or b"")

    report = {
        "status": resp.status_code,
        "body_len": len(raw),
        "blob_hits": len(hits),
        "hits": hits[:20],
        "has_client_message_id": b"s:client_message_id" in (raw or b""),
    }
    if hits:
        from pigeon_protocol.foundation.ws_inner_bootstrap import ingest_binary_inners

        ingested = ingest_binary_inners(session, raw, source="scan_init")
        report["ingested_classes"] = ingested
    OUT.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps(report, ensure_ascii=False, indent=2))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
